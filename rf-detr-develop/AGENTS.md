...
- Optional frequency refinement is selected with `ModelConfig.frm_levels`; a learnably scaled FRM branch runs in
  parallel with the corresponding `MultiScaleProjector` output after per-level C2f projection and before positional
  encoding / transformer flattening.
